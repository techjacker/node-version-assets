...
var Version = function (opts) {
	...
	if (!_.isObject(opts)) {
		console.log('an opts obj must be passed to main constructor', opts);
		throw new Error({message: 'an opts obj must be passed to main constructor'});
	}

	if (!_.isArray(opts.assets)) {

		console.log('files array is missing', opts);
