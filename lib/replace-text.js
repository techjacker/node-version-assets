...
var prependPath = function(path, filePath) {
	if (!path) { return filePath; }
	if(path.endsWith('/') || filePath.startsWith('/')) { return path + filePath; }
...
ReplaceText.prototype.run = function (haystack, overrideRequireJs) {
	...
	haystack = haystack.replace(
		this.fileNameRegex, function (str, p1, p2) {
			// relative paths
			if(p2.startsWith('.')) {
	...
	if (this.requireJs && overrideRequireJs !== true) {
		...
		haystack = haystack.replace(this.fileNameRegex, '$1' + '$2' + newfileNameStem + this.fileNameSuffix);
	}
