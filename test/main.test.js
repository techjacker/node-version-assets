...
cleaner(function (event) {
	...
	test('Main Contructor Fn: shd assign correct properties', function(t) {
		...
		t.equal(mainInstanceDefaultsChecking.grepFiles, undefined);
	...
	test('Main.run(): md5 versioning', function(t) {
		...
		var	mainMd5 = new Main(_.extend({}, opts, {newVersion: null})),
			callMe = function (err, results) {
				...
				// t.deepEqual(cssDirContents, [cssFile], "css directory contents are correct");
				...
				mainMd5.run(function () {
					...
					// t.deepEqual(cssDirContentsSecondTime, [cssFile], "css directory contents are correct");
